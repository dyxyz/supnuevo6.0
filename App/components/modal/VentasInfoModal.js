...
class VentasInfoModal extends Component{
    ...
    renderRow(rowData){
        ...
        var pictureurl ="https://supnuevo.s3.sa-east-1.amazonaws.com/"+ rowData.pictureurl;

        if(rowData.pictureurl!=null && rowData.pictureurl!=undefined && rowData.pictureurl!=''){
            var row =
                <SwipeRow
                    style={{marginTop:5}}
                    preview={true}
    ...
    fetchData_ventas() {
        ...
        proxy.postes_ventas({
            ...
            if(json.re==1) {
                proxy.postes_ventas({
                    url: Config.server2 + "/func/sale/getAllVentasInfoFormNew",
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: {
